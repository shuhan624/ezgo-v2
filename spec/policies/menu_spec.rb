...
RSpec.describe MenuPolicy, type: :policy do
  ...
  %i(index? show? new? create? edit? update? destroy?).each do |action|
    permissions action do
      it "CW Admin 有權限可以 #{action} on Menu" do
        expect(subject).to permit(cw_admin, menu)
      end

      it "沒有權限的 Admin 無法 #{action} Menu" do
        expect(subject).not_to permit(no_permissions_admin, menu)
      end

      it "有權限的 Admin 可以 #{action} on Menu" do
        expect(subject).to permit(admin_has_permissions, menu)
      end
    end
  end
end
