...
RSpec.describe "中文版 - 自訂頁面內頁", type: :system do
  context "網址" do
    it "可以訪問公開的自訂頁面" do
      custom_page = create(:custom_page, :published_tw)
      visit "http://localhost/#{custom_page.slug}"
      expect(page).to have_content(custom_page.title)
    end

    it "無法訪問未公開的自訂頁面" do
      custom_page = create(:custom_page, :hidden_tw)
      visit "http://localhost/#{custom_page.slug}"
  ...
  context "內容" do
    ...
    it "可以看到「文章內容」" do
      custom_page = create(:custom_page, :published_tw, :info)
      visit custom_page_path(custom_page.slug)
      expected_content = find('.container .custom-editor')
      expect(expected_content).to have_content(custom_page.content)
    end
  end
  ...
  context "Meta Data" do
    context "Meta Title" do
      it "當有輸入時，顯示自訂 Meta Title" do
        ...
        custom_page = create(:custom_page, :published_tw, :with_seo)
        visit custom_page_path(custom_page.slug)
      ...
      it "當未輸入時，顯示最新消息「標題」" do
        ...
        custom_page = create(:custom_page, :published_tw)
        visit custom_page_path(custom_page.slug)
        expect(page).to have_title("#{custom_page.title} | #{site_title}", exact: true)
      end
    end
