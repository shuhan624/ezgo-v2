...
RSpec.describe "刪除單一最新消息分類", type: :system do
...
#
  ...
  context "在單一最新消息分類編輯頁" do
    ...
    it "有 destroy 權限者，可以「刪除」最新消息分類" do
      login_as(@admin_has_permissions, scope: :admin)
      destroy_category_on_edit_page
      notice = find('.bootstrap-notify-container')
      expect(notice).to have_content('成功删除 最新消息')
      count = ArticleCategory.count
...
#
  ...
  def destroy_category_on_edit_page
    categories = create_list(:article_category, 5)
    target_category = categories.first
    visit edit_admin_article_category_path(target_category)
    find('.float-btns .btn-danger-light', match: :first).click
    page.driver.browser.switch_to.alert.accept
  end

  def destroy_category_on_show_page
    categories = create_list(:article_category, 5)
    target_category = categories.first
    visit admin_article_category_path(target_category)
    find('.float-btns .btn-danger-light', match: :first).click
    page.driver.browser.switch_to.alert.accept
  end
end
