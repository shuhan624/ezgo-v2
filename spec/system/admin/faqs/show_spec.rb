...
RSpec.describe "檢視單一常見問題頁面", type: :system do
  ...
  context "頁面內容" do
    ...
    context "上方固定顯示內容" do
      ...
      it "可以看到「更新時間」" do
        ...
        expect(page).to have_text(faq.updated_at.strftime("%F %R"))
      end

      it "可以看到「建立時間」" do
        expect(page).to have_text('建立於')
        expect(page).to have_text(faq.created_at.strftime("%F %R"))
      end
    end
    ...
    context "「中文」頁籤" do
      ...
      context "可以看到「發布狀態」" do
        it "當常見問題為「公開」狀態時，可以看到「綠色」圖示" do
          ...
          visit admin_faq_path(published_faq)
          find('.tabs-group .tabs-btn').click_link('中文')
        ...
        it "當常見問題為「未公開」狀態時，可以看到「灰色」圖示" do
          ...
          visit admin_faq_path(hidden_faq)
          find('.tabs-group .tabs-btn').click_link('中文')
    ...
    context "「英文」頁籤" do
      let(:faq) { create(:faq, :published_en) }

      it "可以看到「標題」" do
        visit admin_faq_path(faq)
        find('.tabs-group .tabs-btn').click_link('英文')
        expect(page).to have_content(faq.title_en)
      end
      ...
      context "可以看到「發布狀態」" do
        ...
        it "當常見問題為「未公開」狀態時，可以看到「灰色」圖示" do
          hidden_faq = create(:faq, :hidden_en)
          visit admin_faq_path(hidden_faq)
          find('.tabs-group .tabs-btn').click_link('英文')
          expect(page).to have_xpath("//img[contains(@src,admin/status_invisible.svg)]")
        end
