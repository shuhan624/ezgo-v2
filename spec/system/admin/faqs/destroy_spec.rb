...
RSpec.describe "刪除單一常見問題", type: :system do
...
#
  ...
  context "在單一常見問題編輯頁" do
    ...
    it "有 destroy 權限者，可以「刪除」常見問題" do
      login_as(@admin_has_permissions, scope: :admin)
      destroy_faq_on_edit_page
      notice = find('.bootstrap-notify-container')
      expect(notice).to have_content('成功删除 常見問題')
      count = Faq.count
...
#
  ...
  context "在單一常見問題檢視頁" do
    it "沒有 destroy 權限者，不可以看到「刪除」按鈕" do
      login_as(@no_permissions_admin, scope: :admin)
      visit admin_faq_path(faq)
      expect(page).not_to have_css('.float-btns .btn-danger-light')
    end
...
#
  ...
  def destroy_faq_on_edit_page
    faqs = create_list(:faq, 5)
    target_faq = faqs.first
    visit edit_admin_faq_path(target_faq)
    find('.float-btns .btn-danger-light', match: :first).click
    page.driver.browser.switch_to.alert.accept
  end

  def destroy_faq_on_show_page
    faqs = create_list(:faq, 5)
    target_faq = faqs.first
    visit admin_faq_path(target_faq)
    find('.float-btns .btn-danger-light', match: :first).click
    page.driver.browser.switch_to.alert.accept
  end
end
