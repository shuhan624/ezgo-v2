...
RSpec.describe "刪除單一首頁輪播", type: :system do
...
#
  ...
  context "在單一首頁輪播編輯頁" do
    ...
    it "有 destroy 權限者，可以「刪除」首頁輪播" do
      login_as(@admin_has_permissions, scope: :admin)
      destroy_home_slide_on_edit_page
      notice = find('.bootstrap-notify-container')
      expect(notice).to have_content('成功删除 首頁輪播')
...
#
  ...
  context "在單一首頁輪播檢視頁" do
    ...
    it "有 destroy 權限者，可以「刪除」首頁輪播" do
      login_as(@admin_has_permissions, scope: :admin)
      destroy_home_slide_on_show_page
      notice = find('.bootstrap-notify-container')
      expect(notice).to have_content('成功删除 首頁輪播')
      count = HomeSlide.count
      expect(count).to eq(4)
...
#
  ...
  def destroy_home_slide_on_edit_page
    home_slides = create_list(:home_slide, 5)
    target_home_slide = home_slides.first
    visit edit_admin_home_slide_path(target_home_slide)
    find('.float-btns .btn-danger-light', match: :first).click
    page.driver.browser.switch_to.alert.accept
  end

  def destroy_home_slide_on_show_page
    home_slides = create_list(:home_slide, 5)
    target_home_slide = home_slides.first
    visit admin_home_slide_path(target_home_slide)
    find('.float-btns .btn-danger-light', match: :first).click
    page.driver.browser.switch_to.alert.accept
  end
end
