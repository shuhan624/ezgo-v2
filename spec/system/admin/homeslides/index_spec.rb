...
RSpec.describe "首頁輪播管理", type: :system do
...
#
  ...
  context "首頁輪播列表 (index)" do
    ...
    it "有 index 權限者，可以看到" do
      login_as(@admin_has_permissions, scope: :admin)
      visit admin_home_slides_path
      expect(page).to have_text("首頁輪播列表")
    end
  end
  ...
  context "頁面內容" do
    ...
    it "可以看到電腦尺寸圖片" do
      ...
      expect(element['src']).to have_content(home_slide.banner.filename.to_s)
    end
    ...
    it "可以看到手機尺寸圖片" do
      ...
      expect(element['src']).to have_content(home_slide.banner_m.filename.to_s)
    end

    context "可以看到「中文」是否公開狀態" do
      it "當首頁輪播為「公開」狀態時，可以看到「綠色」圖示" do
      ...
      it "當首頁輪播為「未公開」狀態時，可以看到「灰色」圖示" do
        unpublished_home_slide = create(:home_slide)
        visit admin_home_slides_path
        expected_content = find('.index-list')
        expect(expected_content).to have_xpath("//img[contains(@src,admin/status_invisible.svg)]")
      end

      it "當首頁輪播為「已過發佈時間」狀態時，可以看到「灰色」圖示" do
      ...
      it "當首頁輪播為「未到發佈時間」狀態時，可以看到「灰色」圖示" do
    ...
    context "可以看到「英文」是否公開狀態" do
      it "當首頁輪播為「公開」狀態時，可以看到「綠色」圖示" do
        published_home_slide = create(:home_slide, :with_banner_en, :published_en)
        visit admin_home_slides_path
        expected_content = find('.index-list')
        expect(expected_content).to have_xpath("//img[contains(@src,admin/status_visible.svg)]")
      end
  ...
  context "分頁區塊" do
    ...
    context "未超過「25」筆資料" do
      before(:all) do
        @home_slides = create_list(:home_slide, 25)
      end
    ...
    context "超過「25」筆資料" do
      before(:all) do
        @home_slides = create_list(:home_slide, 26)
      end
      ...
      it "按下「指定分頁」按鈕，可以看到指定的分頁頁面" do
        ...
        click_link('2')
        expect(page).to have_current_path(admin_home_slides_path(page: 2))
      ...
      it "按下「上一頁」按鈕，可以看到目前分頁的上一頁頁面" do
        visit admin_home_slides_path(page: 2)
        scroll_to(find('ul.pagination'), align: :bottom)

        find('ul.pagination li.prev a').click
        expect(page).to have_current_path(admin_home_slides_path)
