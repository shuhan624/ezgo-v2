...
RSpec.describe "合作夥伴管理", type: :system do
...
#
  ...
  context "頁面內容" do
    ...
    it "可以看到中文圖片" do
      partner = create(:partner, :with_image)
      visit admin_partners_path
      element = find('tr td:nth-child(2) img')
      expect(element['src']).to have_content(partner.image.filename.to_s)
    end

    it "可以看到英文圖片" do
      partner = create(:partner, :with_image_en)
      visit admin_partners_path
      element = find('tr td:nth-child(3) img')
      expect(element['src']).to have_content(partner.image_en.filename.to_s)
    end

    it "可以看到合作夥伴「中文」名稱" do
      partner
      visit admin_partners_path
      expected_content = find('.index-list')
      expect(expected_content).to have_content(partner.name)
    end

    it "可以看到合作夥伴「英文」名稱" do
      partner
      visit admin_partners_path
      expected_content = find('.index-list')
      expect(expected_content).to have_content(partner.name_en)
    end

    context "可以看到「中文」是否公開狀態" do
      it "當合作夥伴為「公開」狀態時，可以看到「綠色」圖示" do
      ...
      it "當合作夥伴為「未公開」狀態時，可以看到「灰色」圖示" do
        hidden_partner = create(:partner, :hidden_tw, :with_image)
        visit admin_partners_path
        expected_content = find('.index-list')
  ...
  context "分頁區塊" do
    ...
    context "未超過「25」筆資料" do
      before(:all) do
        @partners = create_list(:partner, 25)
      end
    ...
    context "超過「25」筆資料" do
      before(:all) do
        @partners = create_list(:partner, 26)
      end
      ...
      it "每一頁不可以看到超過「25」筆資料" do
        partner = Partner.all.last # 取得第 26 筆資料
        visit admin_partners_path
        scroll_to(find('table'), align: :bottom)
        expected_content = find('table')
        expect(expected_content).not_to have_content(partner.name)
      ...
      it "按下「指定分頁」按鈕，可以看到指定的分頁頁面" do
        ...
        click_link('2')
        expect(page).to have_current_path(admin_partners_path(page: 2))
      ...
      it "按下「上一頁」按鈕，可以看到目前分頁的上一頁頁面" do
        visit admin_partners_path(page: 2)
        scroll_to(find('ul.pagination'), align: :bottom)
