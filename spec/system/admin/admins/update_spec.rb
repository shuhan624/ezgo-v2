...
RSpec.describe "編輯單一管理帳號頁面", type: :system do
  let(:admin) { create(:admin, name: 'cw_admin') }
  ...
  before(:all) do
    ...
    @no_edit_permit_role = create(:role,
      name: 'no_edit_permit_role',
      permissions: {
        admin: {
          index: true,
          show: true,
...
#
  ...
  context "按鈕區塊" do
    ...
    let(:destroy_permission_role) { create(:role, permissions: {
      admin: { index: true, show: true, edit: true, destroy: true }
    }) }
...
#
  ...
  context '更新管理帳號' do
    ...
    it "有 update 權限者，可以更新" do
      ...
      update_data_on_admin_form
      click_button("更新管理帳號")
      expect(page).to have_content('新的管理者')
      expect(page).to have_content('成功更新 管理帳號')
    end
  end
...
#
  ...
  def update_data_on_admin_form
    ...
    fill_in("admin[name]", with: name)
    fill_in("admin[email]", with: email)
    fill_in("admin[password]", with: password)
    fill_in("admin[password_confirmation]", with: password)


    within('fieldset.radio_buttons.admin_account_active') do
      choose(I18n.t('simple_form.options.admin.account_active.true'), allow_label_click: true)
    end
    select(@permit_role.name, from: 'admin_role_id', visible: false)
