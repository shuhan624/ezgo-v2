...
RSpec.describe "編輯單一轉址頁面", type: :system do
...
#
  ...
  context "編輯轉址頁面 (edit)" do
    ...
    it "有 edit 權限者，可以到達" do
      ...
      visit edit_admin_redirect_rule_path(redirect_rule)
      expect(page).to have_content("編輯轉址")
    end
  end

  context "按鈕區塊" do
    let(:edit_permission_role) { create(:role, permissions: {
      redirect_rule: { index: true, edit: true, }
    }) }
    ...
    let(:destroy_permission_role) { create(:role, permissions: {
      redirect_rule: { index: true, show: true, edit: true, destroy: true }
    }) }
...
#
  ...
  context '更新轉址' do
    it "沒有 update 權限者，不可以更新" do
      ...
      update_data_in_redirect_rule_form
      click_button("更新轉址")
      expect(page).to have_content("無權進行此操作")
    end
...
#   ▀         ▀  ▀▀▀▀▀▀▀▀▀▀▀       ▀       ▀         ▀  ▀▀▀▀▀▀▀▀▀▀▀  ▀▀▀▀▀▀▀▀▀▀
  ...
  def update_data_in_redirect_rule_form
    redirect_rule
